...
import DeleteOrderButton from "./ui/DeleteOrderButton";
...
export default async function OrdersPage({
  ...
  const where: any = { docType: "PURCHASE_CONTRACT" };
        ...
        <table className="w-full text-sm">
          <thead className="text-slate-300">
            <tr className="border-b border-slate-700">
              ...
              <th className="p-3 text-left">Lieferdatum</th>
            ...
            {orders.map((order, idx) => {
              ...
              return (
                <tr key={order.id} className="border-b border-slate-700 last:border-b-0">
                  ...
                  <td className="p-3">
                    <div className="font-semibold text-cyan-300">{order.docNumber}</div>
                    <StatusBadge isFinal={order.isFinal} />
                  </td>
                  <td className="p-3 text-slate-200">{customerName}</td>
                  <td className="p-3 text-slate-200">{vehicleLabel}</td>
                  <td className="p-3 text-slate-300">{formatDate(order.issueDate)}</td>
                  <td className="p-3 text-slate-300">{formatDate(order.deliveryDate)}</td>
                  <td className="p-3 text-right text-slate-200">
                    {formatMoney(order.grossTotalCents)}
                  </td>
                  <td className="p-3 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      {showDelete ? <DeleteOrderButton orderId={order.id} /> : null}
                    </div>
                  </td>
                </tr>
              );
