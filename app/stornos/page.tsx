...
function StatusBadge({ status, isFinal }: { status: string; isFinal: boolean }) {
...
export default async function StornosPage({
  searchParams,
}: {
  searchParams?:
  ...
  const where: any = { docType: "STORNO" };

  if (statusFilter === "draft") where.isFinal = false;
  if (statusFilter === "final") where.isFinal = true;
  if (statusFilter === "paid") where.status = DocumentStatus.PAID;
  if (statusFilter === "cancelled") where.status = DocumentStatus.CANCELLED;
  ...
  if (q) {
    where.OR = [
      ...
      { creditFor: { docNumber: { contains: q } } },
      { customer: { name: { contains: q } } },
    ];
  }

  const stornos = await prisma.document.findMany({
    where,
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-2 rounded border border-slate-800 bg-slate-900/50 px-4 py-2 text-xs text-slate-200 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="text-xs text-slate-400">Erstellt aus bezahlten Rechnungen</div>
      </div>
        ...
        <table className="w-full min-w-[900px] text-sm">
          <thead className="text-slate-300">
            <tr className="border-b border-slate-700">
              ...
              <th className="p-3 text-left">Storno-ID</th>
              <th className="p-3 text-left">Bezug</th>
          ...
          <tbody>
            {stornos.map((s, idx) => {
              const customerName =
                s.customer?.name || (s.customer?.isBusiness ? "Gewerbekunde" : DASH);
              const reference = s.creditFor?.docNumber ? `Rechnung ${s.creditFor.docNumber}` : DASH;

              return (
                <tr key={s.id} className="border-b border-slate-700 last:border-b-0">
                  <td className="p-3 text-slate-400">{idx + 1}</td>
                  <td className="p-3">
                    <div className="font-semibold text-cyan-300">{s.docNumber}</div>
                    <StatusBadge status={s.status} isFinal={s.isFinal} />
                  </td>
                  <td className="p-3 text-slate-300">{reference}</td>
                  <td className="p-3 text-slate-200">{customerName}</td>
                  <td className="p-3 text-slate-300">{formatDate(s.issueDate)}</td>
                  <td className="p-3 text-right text-slate-200">{formatMoney(s.grossTotalCents)}</td>
                  <td className="p-3 text-right">
                    <div className="flex justify-end gap-2">
                      <ActionLink href={`/documents/${s.id}/edit`} title={"\u00d6ffnen"} tone="cyan">
                        <IconSearch />
                      </ActionLink>
                      <ActionLink href={`/api/documents/${s.id}/pdf`} title="PDF" tone="indigo">
                        <IconDownload />
                      </ActionLink>
                    </div>
                  </td>
