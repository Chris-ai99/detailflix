...
export default async function CreditNotesPage({
  ...
  const where: any = { docType: "CREDIT_NOTE" };
        ...
        <table className="w-full min-w-[900px] text-sm">
          <thead className="text-slate-300">
            <tr className="border-b border-slate-700">
              ...
              <th className="p-3 text-left">Gutschrift-ID</th>
          ...
          <tbody>
            {creditNotes.map((cn, idx) => {
              const customerName =
                cn.customer?.name || (cn.customer?.isBusiness ? "Gewerbekunde" : DASH);
              const reference = cn.creditFor?.docNumber
                ? `Rechnung ${cn.creditFor.docNumber}`
                : DASH;

              return (
                <tr key={cn.id} className="border-b border-slate-700 last:border-b-0">
                  <td className="p-3 text-slate-400">{idx + 1}</td>
                  <td className="p-3">
                    <div className="font-semibold text-cyan-300">{cn.docNumber}</div>
                    <StatusBadge status={cn.status} isFinal={cn.isFinal} />
                  </td>
                  <td className="p-3 text-slate-300">{reference}</td>
                  <td className="p-3 text-slate-200">{customerName}</td>
                  <td className="p-3 text-slate-300">{formatDate(cn.issueDate)}</td>
                  <td className="p-3 text-right text-slate-200">{formatMoney(cn.grossTotalCents)}</td>
                  <td className="p-3 text-right">
                    <div className="flex justify-end gap-2">
                      <ActionLink href={`/documents/${cn.id}/edit`} title={"\u00d6ffnen"} tone="cyan">
                        <IconSearch />
                      </ActionLink>
                      <ActionLink href={`/api/documents/${cn.id}/pdf`} title="PDF" tone="indigo">
                        <IconDownload />
                      </ActionLink>
                    </div>
                  </td>
