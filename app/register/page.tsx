  ...
  switch (status) {
    ...
    case "approved":
      return {
        tone: "ok",
        message:
          "Freigabe-Link wurde erstellt und an info@autoaufbereitung-bi.de gesendet (24h gueltig).",
      };
    case "approval":
      return {
        tone: "error",
    ...
    case "mismatch":
      return {
        tone: "error",
        message: "Passwort und Passwort-Wiederholung stimmen nicht ueberein.",
      };
    default:
      return null;
  }
    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-900 px-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-700/70 bg-slate-800/90 p-8 shadow-2xl shadow-slate-950/60">
        ...
        <h1 className="mb-1 text-center text-2xl font-semibold text-slate-100">
          {hasToken ? "Konto erstellen" : "Freigabe anfragen"}
        </h1>
        <p className="mb-6 text-center text-sm text-slate-400">
          {hasToken
            ? "Freigabe bestaetigt. Bitte Passwort setzen und Konto abschliessen."
        ...
        {hasToken ? (
          <form action="/api/auth/register/complete" method="post" className="space-y-4">
